...
def _template_digest(today: str, paper: Paper | None, diagnostics: dict[str, str] | None = None) -> DigestData:
    diagnostics = diagnostics or {}
    if not paper:
        diag_text = (
            f"fallback level: {_safe(diagnostics.get('fallback_level', 'N/A'))}; "
            f"Semantic Scholar: {_safe(diagnostics.get('semantic_scholar_count', '0'))}; "
            f"OpenAlex: {_safe(diagnostics.get('openalex_count', '0'))}; "
            f"arXiv: {_safe(diagnostics.get('arxiv_count', '0'))}; "
            f"deduped: {_safe(diagnostics.get('deduped_count', '0'))}; "
            f"top candidates: {_safe(diagnostics.get('top_candidates', 'N/A'))}"
        )
        sections = {
            "今日论文": "今日未检索到高相关新论文。",
            ...
            "对我自己课题的启发": f"检查检索源连通性与关键词覆盖。诊断信息：{diag_text}",
            "如果要精读，优先看哪几部分": "暂无。",
...
def generate_digest(paper: Paper | None, diagnostics: dict[str, str] | None = None) -> DigestData:
    ...
    if paper and api_key:
        ...
        if llm_result:
            return llm_result
    return _template_digest(today, paper, diagnostics=diagnostics)
